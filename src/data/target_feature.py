...
def generate_target_feature(tickers, tickers_dir_path, target_var_path):
    ...
    for ticker in tickers:
        ticker_df = pd.read_csv(
            f'{tickers_dir_path}/{ticker}.csv', index_col=date_index_label
        )

        shifted_close = np.pad(
            ticker_df['Close'], pad_width=(0, 1), constant_values=(np.NaN, np.NaN)
        )[1:]
        ...
        buy_or_sell_df = pd.DataFrame(
            {'BuySell': buy_or_sell * 1}, index=ticker_df.index
        )
